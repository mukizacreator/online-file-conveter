...
const transporter = nodemailer.createTransport({
  ...
  secure: false,
  auth: {
    user: process.env.EMAIL_USER,   // afe374001@smtp-brevo.com
...
app.post("/api/send-code", async (req, res) => {
  try {
    ...
    // Use a shorter timeout (6 seconds) to avoid long delays
    const emailPromise = transporter.sendMail(mailOptions);
    const timeoutPromise = new Promise((_, reject) => {
      setTimeout(() => reject(new Error('Email timeout - but code is saved in logs')), 6000);
    });

    await Promise.race([emailPromise, timeoutPromise]);
    console.log(`✅ Verification email sent to ${email} via Brevo SMTP`);
    return res.json({ success: true, message: "Verification code sent to your email." });
    
  } catch (error) {
    console.error("❌ Email error:", error.message);
    
    // Always return the code in the response so user can proceed
    ...
    return res.json({ 
      ...
      fallback: true,
      code: code // This allows the user to proceed even if email fails
    });
  }
});
