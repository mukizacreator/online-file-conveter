  ...
  profilePhotoInput.addEventListener("change", function(e) {
    ...
    reader.onprogress = function(event) {
      if (event.lengthComputable) {
        ...
        document.querySelectorAll('.toast-message').forEach(el => {
          if (el.textContent.includes('Uploading photo')) {
            el.textContent = `📸 Uploading photo... ${percent}%`;
          }
        });
      }
    };
    ...
    reader.onload = async function(event) {
      ...
      try {
        ...
        document.querySelectorAll('.toast-message').forEach(el => {
          if (el.textContent.includes('Uploading photo')) {
            el.textContent = '📸 Uploading photo... 50%';
          }
        });
        ...
        document.querySelectorAll('.toast-message').forEach(el => {
          if (el.textContent.includes('Uploading photo')) {
            el.textContent = '📸 Uploading photo... 100%';
          }
        });
...
/* ============================================
   SAVE ACCOUNT - MONGODB (FIXED - FORCE UPDATE)
   ============================================ */
if (saveAccountBtn) {
  saveAccountBtn.addEventListener("click", async function() {
    ...
    try {
      ...
      // FORCE profile view refresh by toggling display
      if (profileView) {
